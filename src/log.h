...
static const int log_level_syslog[] = {LOG_DEBUG, LOG_INFO, LOG_WARNING, LOG_ERR};
...
log_msg(enum log_level level, const char *file, int line, const char *fmt, ...)
{
    ...
    fprintf(g_log_file, "%s.%03ld [%-5s] ", timebuf, ts.tv_nsec / 1000000, log_level_str[level]);
...
#define LOG_DEBUG(fmt, ...) log_msg(LOG_LVL_DEBUG, __FILE__, __LINE__, fmt, ##__VA_ARGS__)
#define LOG_INFO(fmt, ...) log_msg(LOG_LVL_INFO, __FILE__, __LINE__, fmt, ##__VA_ARGS__)
#define LOG_WARN(fmt, ...) log_msg(LOG_LVL_WARN, __FILE__, __LINE__, fmt, ##__VA_ARGS__)
#define LOG_ERR(fmt, ...) log_msg(LOG_LVL_ERR, __FILE__, __LINE__, fmt, ##__VA_ARGS__)
